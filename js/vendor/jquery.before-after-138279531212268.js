...
//             $(obj).prepend('<div id="dragwrapper'+randID+'" class="dragwrapper"><div id="drag'+randID+'"><i></i><img width="49" height="54" alt="handle" src="'+o.imagePath+'handle.png" id="handle'+randID+'" /></div></div>'); // Create drag handle
//             $('#dragwrapper'+randID).css({'opacity':.65,'position':'absolute','padding':'0','left':(imgWidth*o.introPosition)-($('#handle'+randID).width()/2)+'px','z-index':'20'}).width($('#handle'+randID).width()).height(imgHeight);
...
//             $('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'}).after("<span class='icon-before'></span>");;
//             $('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'}).after("<span class='icon-after'></span>");;
//             // $('#handle'+randID).css({'z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'0px'})
//             $('#handle'+randID).css({'z-index':'100','position':'relative','cursor':o.cursor,'top':290+'px','left':'0px'})
...
//                 $('div:eq(2)', obj).width( parseInt( $(this).css('left') ) + 25 );
//             }
...
//                 setTimeout(function(){
//                     $('#dragwrapper'+randID).css({'opacity':1}).animate({'left':(imgWidth*o.introPosition)-($('#dragwrapper'+randID).width()/2)+'px'},o.introDuration,function(){$('#dragwrapper'+randID).animate({'opacity':.65},1000)});
...
//                         $('#dragwrapper'+randID).animate({'opacity':.65},350);
//                     }
//                 );
...
(function($){
    jQuery.fn.compareImages = function(options) {
        for ( var i = 0; i<this.length; i++ ){
            var handler = document.createElement('DIV'),
                wrapper = document.createElement('DIV'),
                help = document.createElement('DIV'),
                copy = this.eq(i).clone(),
                left = 0,
                width = 0;
            
            // styling elements
            copy.css({ position:'relative', width: width + 'px' });
            $(wrapper).css({ position:'absolute', width:'50%', 'z-index':'1000'});
            $(help).css({ width:'100%', overflow:'hidden', 'z-index': '1', position: 'relative' });
            $(handler).css({ position:'absolute', right:'0' }).addClass('handler');
            
            // creating new html structure
            copy.children().eq(0).wrap(wrapper).wrap(help);
            $(handler).append('<div/>');
            copy.children().eq(0).append(handler);
            
            // attaching event
            copy.on( 'mousemove', function(e){
                var left = $(this).offset().left,
                    right = left + $(this).children().eq(1).width(),
                    x = e.pageX;
            ...
            // applying new structure to the document
            this.eq(i).replaceWith(copy);
            
            // applying comtainer width
            help = $('img', copy);
            help.eq(0).width() >= help.eq(1).width() ?  copy.width( help.eq(0).width() ) : copy.width( help.eq(1).width() )
        }

        $('img[alt=after]').after("<span class='icon-after'></span>");
        $('img[alt=before]').after("<span class='icon-before'></span>");
